const app = new Vue ({
    ...
    methods: {
        ...
        nuevoProductoCargar () {
            ...
            if (this.errores.errorNombre == false && this.errores.errorPrecio == false && this.errores.errorStock == false) {
                ...
                fetch(url, options)
                    .then(() => {
                        this.productos = []
                        var url1 = 'http://localhost:5000/productos'
                        this.fetchData(url1)
                    })
                    .catch(err => {
                        this.errored = true
        ...
        cargarModificar (x,y) {
            this.modificarID = y
            this.mostrarNuevo(x)
            let url = "http://localhost:5000/productos/"+y
            var options = {
                method: 'GET',
        ...
        modificar () {
            this.validar()
            if (this.errores.errorNombre == false && this.errores.errorPrecio == false && this.errores.errorStock == false) {
                let url = "http://localhost:5000/productos/"+this.modificarID
                let producto;
        ...
        eliminar (producto) {
            var url = 'http://localhost:5000/producto/' + producto;
            var options = {
                method: 'DELETE',
                headers: {
                    "Content-Type": "application/json"
    ...
    created() {
        var url = 'https://sql10.freesqldatabase.com/sql10503816/productos'
        this.fetchData(url)
    },
    })